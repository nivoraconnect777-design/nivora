...
export default function StoryTray({ isDark }: StoryTrayProps) {
    ...
    // Listen for updates from StoryViewer
    useEffect(() => {
    ...
    const checkHasUnseen = (userStories: any[]) => {
    ...
    const myStories = hasMyStory ? storyUsers[myStoryIndex].stories : [];
    const hasMyUnseen = checkHasUnseen(myStories);
            ...
            <div className="flex items-center gap-4 overflow-x-auto no-scrollbar h-full px-2">
                ...
                {isLoading ? (
                    ...
                    otherStories.map((storyUser: StoryUser, index: number) => {
                        const hasUnseen = checkHasUnseen(storyUser.stories);
                        return (
                            <motion.button
                                ...
                                className="flex flex-col items-center gap-1 min-w-[80px]"
                            >
                                <div className={`w-20 h-20 rounded-full p-[3px] ${hasUnseen
                                    ? 'bg-gradient-to-tr from-yellow-400 via-orange-500 to-fuchsia-600'
                                    : isDark ? 'bg-gray-700' : 'bg-gray-300'
                                    }`}>
